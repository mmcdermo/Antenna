...
class ResourceManager(object):
    ...
    def _cluster_name(self):
        return self._controller.project_name + "Cluster"
    ...
    def aws_resources_needed(self):
        ...
        for source_config in self._controller.config['sources']:
            resources += self._controller.instantiate_source(source_config).external_resources()
    ...
    def default_dynamo_tables(self, context):
        ...
        """
        source_state_config = ResourceManager.dynamo_key_schema("source_state_key")
        ...
        article_reference_config = {
            "key_schema": OrderedDict([
                ('article_url_key', 'HASH'),
            ]),
            "attribute_definitions": {
                'article_url_key': 'S',
            }
        }
        article_reference_table = r.DynamoDBTableResource(
            context, self.dynamo_table_name("article_reference"),
            attribute_definitions=article_reference_config['attribute_definitions'],
            key_schema=article_reference_config['key_schema'],
            write_units=4, read_units=4
        )

        return [source_state_table, article_reference_table]
    ...
    def create_resource_cluster(self):
        ...
        for item_type in self._controller.item_types():
            queues[item_type] = r.SQSQueueResource(context, self.queue_name(item_type))
        ...
        resources = self.aws_resources_needed() + self.default_dynamo_tables(context)
        ...
        self.lambdaRole = r.IAMRoleResource(context,
